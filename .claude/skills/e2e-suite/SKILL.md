...
| `npm run probe:android-copy-picker` | the batch move's which-copies picker rows on the real Android webview — full-grain labels (foil/board/condition/language telling two stacks of one binder apart), 44 px ± targets on the device's own dpr, a real `Input.dispatchTouchEvent` on `+` raising that row's count and the total with it, the stepper stopping at the stack's size, and no sideways scroll. Via the bench: the dialog's hosting pages are authed and the dev webview cannot sign in (verified — the login form answers "Something went wrong"), and this control is `hidden`-free precisely because `CountStepper`'s ± are not (P6-150) |
