...
| Bottom sheet (grabber, scrim, sticky action) | MCard | `sheet` | Direct (grabber styling ours). MFil removed from this row — ruling 2026-08-15: the mobile filter sheet opens as a **left slide-over** (`SheetDirection::Left`; full height suits the long filter list), overruling the frame's bottom-sheet drawing |
...
| `sheet` | Adopt with deviations | Same skeleton as dialog: SSR-safe closed markup; counter ID (sheet.rs:40); ScrollLock; JS-only open/close. Bottom variant is a fixed `h-[400px]` panel — the mobile card sheet chrome (grabber, drag-to-dismiss, snap heights) is ours on top. (The mobile filter sheet uses the left variant by ruling, 2026-08-15.) |
