...
- **Validate** — every PR plus pushes to `main`, linux only (`blacksmith-4vcpu-ubuntu-2404`; `push` is scoped to `main` because a bare `on: push` alongside `on: pull_request` fires two identical runs per branch push): `test -L .agents/skills` (the skill-tree symlink stays a symlink — specs/ui-work-loop.md), `cargo fmt --check`, clippy over the workspace with `-D warnings` (ssr and wasm targets), `cargo build -p three_rings` (debug; codegen-time coverage for the native-only view tree — a `#![recursion_limit]` overflow is invisible to `check`/clippy, see Findings), `cargo test`, `cargo leptos build --release` (exercises the Tailwind pipeline). CI installs the Tauri linux system deps, so `src-tauri` is linted (and, since 2026-08-11, built) here even though the devcontainer excludes it. Rust caching throughout via `useblacksmith/rust-cache` (Blacksmith's sticky-disk fork of Swatinem/rust-cache).
- **Artifacts** — never per-push on branches (agent iteration stays cheap). Both native targets are **`workflow_dispatch` only** (revised 2026-07-16, superseding the Android-on-merge cadence): early in dev most merges don't need a fresh native build, so minting one is a deliberate act, not a per-merge default. A single dispatch carries two boolean inputs — `android` (default `true`) and `macos` (default `false`) — gating each job, so the common case (fire an APK) spins only the cheap linux runner while macOS is opt-in per run. The maintainer clicks "Run workflow" (or asks the agent to fire it via the GitHub API) when a checkable build is wanted.
  - **Android APK** — gated on the `android` input. x86_64 linux runner (`blacksmith-4vcpu-ubuntu-2404` — must stay x64: Blacksmith's arm64 shapes would reintroduce the linux-arm64 NDK gap that pushed local Android builds host-side). Use **NDK r28+**, which defaults to 16 KB page alignment and closes that spike finding. `cargo tauri android build --apk --target aarch64`. Roughly 30 free-tier minutes per run.
  - **macOS `.dmg`** — gated on the `macos` input (default off). Blacksmith macOS runner (`blacksmith-6vcpu-macos-latest`, Apple Silicon M4) at $0.08/min — ~20× the free-tier basis rate — so it stays opt-in: the `.dmg`'s consumer is a human at a Mac (who can tick the box right there), and pre-merge desktop checks happen as local host builds anyway. Unsigned/ad-hoc; opened via right-click → Open.
...
- [x] `workflow_dispatch` with the `android` input → rolling `latest` release carries a fresh APK (installs in place over the previous one). *(Originally per-merge; gated to dispatch-only 2026-07-16 — see Design § Artifacts.)*
...
*(Native build gate, 2026-08-11 — P6-089.)*

- **The gate was structurally blind to `#![recursion_limit]` regressions.** Diagnosed 2026-07-26 (rustc 1.96.1): the overflow is resolved while computing the layout of a monomorphized async block, a codegen-time rustc query that `cargo check`/clippy never runs — so no clippy line, however thorough, can catch it, and the existing `cargo leptos build --release` step only codegens the wasm instantiation. What was genuinely uncovered is narrow: a view tree reachable *solely* under `--features native` (i.e. only inside `src-tauri`). Fix: one `cargo build -p three_rings` step. Debug profile is enough — codegen, not optimization, is what's exercised, and it's the cheapest build that reaches it. No new CI tooling needed: the runner already installs Tauri's Linux system deps for the workspace clippy line. rustc names `app` as the crate to fix regardless of which of `app`/`frontend`/`three_rings` triggers the overflow, since the offending generic originates in `app`. Headroom at the committed limit (`app/src/lib.rs`, `512`) is 2×: fails at 128, clean at 256.
