...
- [ ] GitHub: create a free org + private repo under it (Blacksmith requires org ownership), push `main`, enable Actions + auto-merge, branch protection requiring the validate check, install the Blacksmith GitHub app, add secrets (`ANDROID_KEYSTORE` + passwords; `RENDER_DEPLOY_HOOK` only if needed) (specs: [delivery-pipeline](delivery-pipeline.md))
