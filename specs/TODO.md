...
- [~] macOS `.dmg` artifact job (`workflow_dispatch` only — Blacksmith macOS at $0.08/min) — authored + correctly `workflow_dispatch`-gated (skips on merge); not yet dispatch-verified (specs: [delivery-pipeline](delivery-pipeline.md))
