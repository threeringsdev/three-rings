...
- [ ] GitHub: create private repo, push `main`, enable Actions + auto-merge, branch protection requiring the validate check, add secrets (`ANDROID_KEYSTORE` + passwords; `RENDER_DEPLOY_HOOK` only if needed) (specs: [delivery-pipeline](delivery-pipeline.md))
