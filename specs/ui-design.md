...
- 2026-08-13 — **Dark-theme contrast pass** (P6-099); deliverable: the `.dark` token block in [`style/input.css`](../style/input.css). This spec itself never recorded the palette's numeric values (they live only in the CSS, per `specs/app-ui.md`'s "Tokens, not hex"); this entry is the ladder's design record. Maintainer complaint: the page background read too dark and card borders blended into it. Measured root cause (OKLCH grayscale → sRGB relative luminance, chroma 0 so hue is inert): `background` (0.18) vs `card` (0.205) was a 1.05:1 luminance ratio — perceptually flat, cards did not lift off the page — and the 10%-white-alpha `border`, composited over either surface, landed at ~2.7:1 and ~2.65:1, under WCAG 1.4.11's 3:1 non-text-contrast floor. Fix, applied only inside `.dark` (light theme untouched): `background` 0.18→0.20; `card` 0.205→0.26 (a real jump off the page rather than a rounding-error one); `popover`/`secondary`/`muted` 0.269→0.30, kept as one shared "surface" tier one step above `card` (as they already were, just raised to match); `accent` 0.371→0.40, keeping its interactive-hover tier separated from that surface tier; `border` alpha 10%→16%, `input` alpha 15%→22%. Alpha-composited borders scale contrast far better than a solid gray step at this low end of the luminance curve — a same-sized solid-gray "two steps up" move barely reached 2:1, while the alpha route cleared 3:1 in both directions — so border/input stayed alpha rather than becoming solid tokens. `ring` (0.556) and `muted-foreground` (0.708) were left unchanged: both already cleared their contrast targets (ring ≈3.8:1 non-text against the new background; muted-foreground ≥5.2:1 against every surface, foreground ≥13:1 everywhere) with the surrounding ladder shifted under them. Verified against the real app, not just computed: Playwright screenshots (`dark-color-scheme` context) of `/catalog`, an authed `/my/collections/:id` (table + header chips), and the `/dev/components` bench's selection tray + a Dialog, captured before (stashed CSS) and after. Pixel-sampled confirmation on the dialog: the border pixel at the panel edge moved from `rgb(42,42,42)` to `rgb(59,59,59)` against a `rgb(18,18,18)`→`rgb(22,22,22)` fill — the border-to-fill spread grew from 24 to 37, a ~54% wider gap. Regression-only e2e (`states.spec.ts` full file + `catalog.spec.ts --grep @fast`, chromium): 12/12 and 32/32 passed — the suite carries no visual assertions, so this is presence/absence-of-breakage evidence, not a substitute for the screenshots.
