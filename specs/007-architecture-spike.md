...
1. Pick scaffold base: [start-tauri-fullstack](https://github.com/rust-ui/start-tauri-fullstack) (Rust/UI pre-wired) vs. [tauri-leptos-ssr](https://github.com/codeitlikemiley/tauri-leptos-ssr) — evaluate both, pick one (see 006).
...
(Record here as the spike proceeds — this section becomes the input to 005 and future specs.)

- Mobile SSR-in-process: does the embedded Axum pattern work on iOS/Android at all, or is mobile CSR-only?
- Where do DB credentials live during the spike? (Native builds talking directly to Neon is acceptable *for the spike only* — spec 005 removes this before any real user data.)
